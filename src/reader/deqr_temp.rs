...
impl DeQR {
    ...
    pub fn set(&mut self, r: usize, c: usize, demod: Pixel) {
        *self.get_mut(r, c) = demod;
    }

    pub(crate) fn get_region(&mut self, src: (usize, usize)) -> Region {
        ...
        match px {
            Pixel::Color(clr) => {
                ...
                Region::Visited { id, area: area.0 }
            }
            Pixel::Visited(id) => Region::Visited { id, area: self.regions[id] },
            Pixel::Finder => Region::Finder,
            Pixel::Alignment => Region::Alignment,
            Pixel::Temporary => Region::Temporary,
        }
    }
